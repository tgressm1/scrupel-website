...
This site mirrors its brand from a separate, unrelated repo — `../scrupel-app`
...
- **Color palette**: this repo's `styles.css` `:root` (light) and
  ...
  `../scrupel-app/src/App.jsx`'s `LIGHT`/`DARK` theme objects.
- **Favicon**: `favicon.png` here is a copy of
  `../scrupel-app/android/app/src/main/res/mipmap-xxxhdpi/ic_launcher.png`.
- **Feature copy** in `index.html`: describes what the app actually does —
  should stay accurate to the real feature set in `../scrupel-app`.

**If asked to change colors, the icon, or feature copy here**, first check
whether `../scrupel-app` changed first (read it directly). If the app changed
...
tweak (e.g. pure copywriting/layout) and doesn't need to touch `../scrupel-app`
at all.
...
established in `../scrupel-app`; it's a general preference for how Tyler works,
